...
const getBetaValue = (s: any): number | null => {
  // 1) якщо betaBucket реально число
  ...
  // 4) best_params.static.beta (якщо прилітає в payload)
...
useEffect(() => {
  ...
  filterReport, accountNonEmptyFirst, equityType,zapMode, zapMinAbs, zapSigmaMinAbs
]);

// Load effect для кнопок
  useEffect(() => {
    try {
  ...
  }, [cls, type, mode, listMode, bpCls, zapMode]);
  ...
  const onTickerClick = (tk: string) => {
    ...
    // НЕ чіпаємо collapsed/mode → користувач вибрав сам
  };
  ...
  const displayPxFor = (s: ArbitrageSignal, side: "short" | "long") => {
    if (side === "short") return { label: "BID", value: toNum(s.bidStock) };
    return { label: "ASK", value: toNum(s.askStock) };
  };
