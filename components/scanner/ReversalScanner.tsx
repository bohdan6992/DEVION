...
// Reversal tracks no sigma metric and no peak (the mapper leaves those slots null and
...
const STRATEGY = defineScannerStrategy({
  ...
  // Reversal has no |sigma| to look a bin up by, so the RATING GATES axes read the gamma/gammaN the
...
const ACTIVE_TICKER_STRATEGY = "reversal" as const;

/**
 * Reversal's exit classes — ReversalTiming.Default.ExitTargetMinByClass: five clock-time exits
 * spanning the evening into the next morning's open print. Lowercase keys exactly as the bridge
 * (PaperReversalRequest.ExitClass) expects them.
 */
const REVERSAL_EXIT_CLASSES = [
...
export default function ReversalScanner({
  ...
  // Reversal exit class — replaces the inherited Arbitrage session-band selector with the five
  // clock-time exits ReversalTiming.Default actually computes (18:00 / 21:00 / 04:00+1 / 07:00+1 /
  ...
  // Range analysis runs entirely client-side for Reversal, same as OpenDoor/Day Two:
  ...
  const REVERSAL_OPTIMIZER_ENABLED = true;
  ...
  const REVERSAL_SCENARIO_RUNNER_ENABLED = true;

  const [reversalExitClass, setReversalExitClass] = useState<"exit18" | "exit21" | "exit04" | "exit07" | "print">("print");
  // ReversalGate.Check's whole rule: |15:50 deviation| must clear BOTH a flat floor (now split by
  // side — see below) AND the ticker's own published gamma for (class, sign) — see ReversalGate.cs.
  ...
  const [reversalMinDevAbsShort, setReversalMinDevAbsShort] = useState(0.5);
  ...
  // reversalMinDevAbsShort), mirroring startAbsNeg's own fallback UX exactly.
  const [reversalMinDevAbsLong, setReversalMinDevAbsLong] = useState("");
  ...
  const [reversalMinDevAbsMax, setReversalMinDevAbsMax] = useState("");
  ...
  const bumpReversalMinDevAbsMax = (delta: number) => {
    setReversalMinDevAbsMax((prev) => {
      const raw = String(prev ?? "").trim();
      const parsed = Number(raw.replace(",", "."));
      const cur = Number.isFinite(parsed) ? parsed : reversalMinDevAbsShort;
  ...
  // ========= REVERSAL SNAPSHOT: live per-day tape replay (architecturally mirrors Arbitrage's
  // SNAPSHOT tab — POST .../episodes/search, per-day cache, day-range build — but hits
  // /api/paper/reversal/episodes/search, gated by ReversalGate's flat-floor + per-ticker gamma and
  // using TapeReversalEngine's fixed 15:50-signal/16:00-entry, clock-time-exit rule instead of
  // Arbitrage's hedge engine. Real per-day accuracy: each row is an ACTUALLY realized trade, not a
  // bin average.
  //
  // Unlike OpenDoor/Day Two there is no bid/ask crossing and no benchmark leg: per the user's own
  ...
  // (reversalSnapshotRows, defined after filteredEpisodes further down). This used to be a second,
  ...
  useFilterRestore(filtersLsKey, filtersHydratedRef, filtersRestoringRef, (s) => {
        ...
        // Reversal gate settings. Restored here rather than left at their defaults so the toolbar
        // shows what is actually being traded after a reload.
        if (REVERSAL_EXIT_CLASSES.some((c) => c.key === s.reversalExitClass)) setReversalExitClass(s.reversalExitClass);
        if (typeof s.reversalMinDevAbsShort === "number") setReversalMinDevAbsShort(s.reversalMinDevAbsShort);
  ...
  const persistedFilters = useMemo(
    () => ({
      ...
      sizingMode,
      sizeValue,
      // Reversal's own gate settings. These used to be plain component state that reset on every
      // reload, so "the filters I trade with" existed only until the tab was refreshed.
      reversalExitClass,
      reversalMinDevAbsShort,
  ...
  const {
    ...
    // NOTE: "ReversalEnterLong"/"ReversalEnterShort" have no TradingAppOrderIntentType entry yet
    // (no ReversalServerStrategy exists — see DayTwoServerStrategy.cs for the pattern to follow).
    // Deliberately NOT reusing "DayTwoEnterLong"/"DayTwoEnterShort" here: that would fire Day
    // Two's own live hotkey mapping (Hotkeys.DayTwoBuy/DayTwoSell) for a Reversal order if manual
    ...
    entryIntentTypes: { long: "ReversalEnterLong", short: "ReversalEnterShort" },
  ...
  // ---- Reversal request shape (PaperReversalRequest) ----
  // The inherited buildGetParams/buildPostRequest below describe an Arbitrage episode search
  // (deviation thresholds, hold windows, close modes). Reversal's engine takes none of that: the
  // signal/entry/exit minutes are fixed by the clock (ReversalTiming.Default), so the only inputs
  // of its own are the exit class, the flat |15:50 deviation| floor and whether to skip the
  // per-ticker gamma lookup entirely.
  function buildReversalParams() {
    ...
    return {
      ...
      // server-side in PaperFilters.PassesStaticMetaFilters, after the per-day cache.
      ...
      // the country string, so "UNITED STATES"/"US" and CHINA/"HONG KONG" both land).
  ...
  function buildReversalGetParams(d: string) {
    return { dateNy: d, ...buildReversalParams() };
  }

  function buildReversalPostRequest(from: string, to: string) {
    return { dateFrom: from, dateTo: to, ...buildReversalParams() };
  }
    ...
    try {
      for (let i = 0; i < combos.length; i++) {
        ...
        const j = await apiPost<any>(
          ...
          { ...buildReversalPostRequest(req.dateFrom, req.dateTo ?? req.dateFrom), includeBestParams: false }
        );
        const rows = normalizeRows<PaperArbClosedDto>(j) ?? [];
  ...
  // All ~78 numeric range boxes below (ADV/VWAP/spread/volatility/etc, same shape as ρ/β/σ above)
  // drove a full re-filter of every row on EVERY keystroke, same problem deferredQTicker already
  // solves for the ticker box. Deferring them one at a time would mean one useDeferredValue call
  // per field; bundling them into a single object and deferring THAT defers the whole group with
  // one call. filteredActive/filteredEpisodes below still read the raw fields directly (unchanged) —
  // only their dependency arrays reference this bundle, so React skips recomputing the filter on the
  ...
  }, [activeRows, deferredQTicker, qSide, listMode, ignoreSet, applySet, pinSet, zapMode, startAbs, ratingMode, metric, ratingRules, session, arbitrageTickerMetaByTicker, sharedRangeFilterModes, deferredRangeFilterBundle, requireHasReport, excludeHasReport, excludeCorr, sectorCorr.excluded, topMode, topSigmaOn, topBenchOn, topTimeOn]);
  ...
  // The SNAPSHOT table predates the shared row shape and reads Reversal's own field names, so
  ...
  // TapeReversalEngine, not a side-dependent fill.
  const reversalSnapshotRows = useMemo<ReversalPaperClosed[]>(
    () =>
      filteredEpisodes.map((r: any) => {
        ...
        return {
          ...
          entryStack: r?.lstPrcLstClsPct ?? null,
          ...
          // 16:00 reading the trade is actually priced from. See ReversalClosed.SignalDev.
  ...
  const reversalSnapshotStats = useMemo(() => {
    const rows = reversalSnapshotRows.filter((r) => r.pnl != null);
    ...
    const sorted = [...reversalSnapshotRows].sort((a, b) => (b.pnl ?? -Infinity) - (a.pnl ?? -Infinity));

    return { rows: sorted, trades, totalPnl, winRate, avgTrade, maxWin, maxLoss, avgWin, avgLoss, profitFactor, expectancy };
  }, [reversalSnapshotRows]);

  // ACTIVE tab: genuinely open Reversal positions (signalled ~15:50, entered ~16:00, whose exit
  ...
  // client-side per-bin rate/total/avg_move research view (/api/opendoor/summary) with no Reversal
  // equivalent: Reversal publishes a single gamma per (ticker, class, sign), not an up/down bin
  // table, so there is nothing to reproduce that panel from. Showing real open positions here is
  // both accurate to the data Reversal actually has and more useful than an empty/fabricated tab.
  const reversalActiveRows = useMemo(
    () =>
      filteredActive.map((r: any) => ({
        ticker: String(r?.ticker ?? ""),
        side: String(r?.side ?? "") === "Long" ? "Long" : "Short",
        entryMinuteIdx: Number(r?.start?.minuteIdx ?? NaN),
        minutesToExit: r?.minutesToExit ?? null,
        exitClass: r?.closeMode ?? null,
        signalDev: r?.start?.metric ?? null,
        entryStack: r?.start?.lstPrcLstClsPct ?? null,
        lastStack: r?.last?.lstPrcLstClsPct ?? null,
  ...
  const reversalActiveStats = useMemo(() => {
    const rows = reversalActiveRows;
    ...
    const avgTrade = withPnl.length > 0 ? totalPnl / withPnl.length : 0;
    const maxWin = wins.length ? Math.max(...wins.map((r) => r.pnl ?? 0)) : 0;
    const maxLoss = losses.length ? Math.min(...losses.map((r) => r.pnl ?? 0)) : 0;
    const avgWin = wins.length ? wins.reduce((s, r) => s + (r.pnl ?? 0), 0) / wins.length : 0;
    const avgLoss = losses.length ? losses.reduce((s, r) => s + (r.pnl ?? 0), 0) / losses.length : 0;
      ...
      <div className="relative z-10 max-w-[1920px] mx-auto space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-white/[0.06] bg-[#0a0a0a]/50 p-3 shadow-xl backdrop-blur-md transition-all duration-300 hover:border-white/[0.12] hover:bg-[#0a0a0a]/70">
            ...
            <div className="flex h-7 items-center gap-2">
              {[
                ...REVERSAL_EXIT_CLASSES,
              ].map((b) => (
                <button
                  key={b.key}
                  type="button"
                  onClick={() => setReversalExitClass(b.key as "exit18" | "exit21" | "exit04" | "exit07" | "print")}
                  className={clsx(
                    TOOLBAR_BUTTON_BASE,
                    reversalExitClass === b.key
        ...
        />
          ...
          <FilterFlagsRow
            ...
            zapSlot={
              <>
              {/* Rendered via zapSlot (not strategySlot) so this group is the LAST thing in the row —
                  past the ABC sort dropdown, at the true right edge, matching ArbitrageScanner's own
                  ...
                  ABC, not before it). */}
              {/* One continuous pill group, styled 1-to-1 off ReversalScout's own ScoutUnitBar (the
              ...
              <div className="inline-flex items-center gap-2 rounded-xl border p-1.5 border-violet-500/20 bg-violet-500/[0.06]">
                {([
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={u.disabled}
                      onClick={() => !u.disabled && setReversalUnitMode(u.key)}
                      title={u.title}
                      className={clsx(
                        ...
                        u.disabled
                          ? "bg-transparent border-transparent text-violet-300/25 cursor-not-allowed"
                          : on
                ...
                <div className="group relative w-[78px] rounded-md border border-[#f3a6b2]/50" title={`Floor on |15:50 deviation| for a POSITIVE reading (a SHORT entry), entered in ${reversalThresholdUnitLabel}. Coral = short.`}>
                  <input
                    ...
                    step={0.1}
                    min={0}
                    value={reversalMinDevAbsShort}
                    onChange={(e) => setReversalMinDevAbsShort(Math.max(0, clampNumber(e.target.value, 0)))}
                  ...
                  <div className="absolute right-[1px] top-[1px] bottom-[1px] w-4 border-l border-white/10 bg-transparent flex flex-col overflow-hidden rounded-r-[5px] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto group-focus-within:opacity-100 group-focus-within:pointer-events-auto transition-opacity">
                    <button
                      type="button"
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => setReversalMinDevAbsShort((v) => Math.max(0, +(v + 0.1).toFixed(4)))}
                      className="flex flex-1 items-center justify-center text-[8px] leading-none text-zinc-500 hover:text-zinc-300 transition-colors"
                ...
                <div className="group relative w-[78px] rounded-md border border-[#6ee7b7]/45" title={`Floor on |15:50 deviation| for a NEGATIVE reading (a LONG entry), entered in ${reversalThresholdUnitLabel}. Empty = the same threshold as the short one. Mint = long.`}>
                  <input
                    ...
                    step={0.1}
                    min={0}
                    value={reversalMinDevAbsLong}
                    onChange={(e) => setReversalMinDevAbsLong(e.target.value)}
                  ...
                  <div className="absolute right-[1px] top-[1px] bottom-[1px] w-4 border-l border-white/10 bg-transparent flex flex-col overflow-hidden rounded-r-[5px] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto group-focus-within:opacity-100 group-focus-within:pointer-events-auto transition-opacity">
                    <button
                      type="button"
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => setReversalMinDevAbsLong((prev) => {
                        const cur = optNumOrNull(prev) ?? reversalMinDevAbsShort;
                        return String(Math.max(0, +(cur + 0.1).toFixed(4)));
                      })}
                      className="flex flex-1 items-center justify-center text-[8px] leading-none text-zinc-500 hover:text-zinc-300 transition-colors"
                      aria-label="Increase long-side floor"
                    >
                      ▲
                    </button>
                    <button
                      ...
                      className="flex flex-1 items-center justify-center text-[8px] leading-none text-zinc-500 hover:text-zinc-300 transition-colors border-t border-white/5"
                      aria-label="Decrease long-side floor"
                    >
                      ▼
                    </button>
                  </div>
                ...
                <div className="group relative w-[78px] rounded-md border border-zinc-200/35" title={`Upper cap on |15:50 deviation|, either side (empty = none), entered in ${reversalThresholdUnitLabel}: a reading beyond it is an anomalous outlier and is not taken. Silver = the ceiling.`}>
                  <input
                    ...
                    step={0.1}
                    min={0}
                    value={reversalMinDevAbsMax}
                    onChange={(e) => setReversalMinDevAbsMax(e.target.value)}
                    placeholder="no cap"
                    className="center-spin w-full h-7 bg-black/20 border-0 rounded-md !pl-2 !pr-5 text-[11px] text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-0 focus:bg-black/30 transition-all active:scale-[0.99] font-mono tabular-nums text-center"
                  />
                  <div className="absolute right-[1px] top-[1px] bottom-[1px] w-4 border-l border-white/10 bg-transparent flex flex-col overflow-hidden rounded-r-[5px] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto group-focus-within:opacity-100 group-focus-within:pointer-events-auto transition-opacity">
                    <button
                      type="button"
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => bumpReversalMinDevAbsMax(0.1)}
                      className="flex flex-1 items-center justify-center text-[8px] leading-none text-zinc-500 hover:text-zinc-300 transition-colors"
                ...
                <div className="group relative w-[78px] rounded-md border border-amber-400/55" title="Sample-size floor on the ticker's own published gamma (GammaN) — a gamma backed by too few historical trades does not qualify. 0 = off. Amber = the sample floor.">
                  <input
                    type="number"
                    inputMode="numeric"
                    step={1}
                    min={0}
                    value={reversalMinGammaTotal}
                    onChange={(e) => setReversalMinGammaTotal(Math.max(0, clampInt(e.target.value, 0)))}
                    className="center-spin w-full h-7 bg-black/20 border-0 rounded-md !pl-2 !pr-5 text-[11px] text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-0 focus:bg-black/30 transition-all active:scale-[0.99] font-mono tabular-nums text-center"
                  />
                  <div className="absolute right-[1px] top-[1px] bottom-[1px] w-4 border-l border-white/10 bg-transparent flex flex-col overflow-hidden rounded-r-[5px] opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto group-focus-within:opacity-100 group-focus-within:pointer-events-auto transition-opacity">
                    <button
                      type="button"
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => setReversalMinGammaTotal((v) => Math.max(0, v + 1))}
                      className="flex flex-1 items-center justify-center text-[8px] leading-none text-zinc-500 hover:text-zinc-300 transition-colors"
        ...
        {primaryPanel === "scanner" && tab === "active" && (
          <div className="space-y-3">
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
              <SummaryMetricCard
                label="TOTAL PNL ($)"
                value={num(reversalActiveStats.totalPnl, 2)}
                className="xl:row-span-2 xl:min-h-[124px]"
                valueClassName={
                  clsx(
                    "text-4xl md:text-6xl font-bold",
                    reversalActiveStats.totalPnl > 0
                      ? "text-[#6ee7b7]"
                      : reversalActiveStats.totalPnl < 0
              ...
              />
              <SummaryMetricCard label="OPEN POSITIONS" value={intn(reversalActiveStats.trades)} inline />
              <SummaryMetricCard label="WIN RATE" value={`${num(reversalActiveStats.winRate * 100, 1)}%`} inline />
              <SummaryMetricCard
                label="AVG ($)"
                value={num(reversalActiveStats.avgTrade, 2)}
                inline
                valueClassName={reversalActiveStats.avgTrade > 0 ? "text-emerald-300" : reversalActiveStats.avgTrade < 0 ? SOFT_LOSS_TEXT_CLASS : "text-zinc-200"}
              />
              <SummaryMetricCard
                label="MAX WIN ($)"
                value={num(reversalActiveStats.maxWin, 2)}
                inline
                valueClassName={reversalActiveStats.maxWin > 0 ? "text-[#6ee7b7]" : "text-zinc-200"}
              />
              <SummaryMetricCard
                label="AVG WIN ($)"
                value={num(reversalActiveStats.avgWin, 2)}
                inline
                valueClassName={reversalActiveStats.avgWin > 0 ? "text-[#6ee7b7]" : "text-zinc-200"}
              />
              <SummaryMetricCard label="PROFIT FACTOR" value={Number.isFinite(reversalActiveStats.profitFactor) ? num(reversalActiveStats.profitFactor, 2) : "∞"} inline />
              <SummaryMetricCard label="EXPECTANCY ($)" value={num(reversalActiveStats.expectancy, 2)} inline />
              <SummaryMetricCard
                label="MAX LOSS ($)"
                value={num(reversalActiveStats.maxLoss, 2)}
                inline
                valueClassName={reversalActiveStats.maxLoss < 0 ? SOFT_LOSS_TEXT_CLASS : "text-zinc-200"}
              />
              <SummaryMetricCard
                label="AVG LOSS ($)"
                value={num(reversalActiveStats.avgLoss, 2)}
                inline
                valueClassName={reversalActiveStats.avgLoss < 0 ? SOFT_LOSS_TEXT_CLASS : "text-zinc-200"}
              />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between mb-2">
                <div className="text-[10px] uppercase tracking-widest font-mono text-zinc-500">
                  OPEN POSITIONS | rows {reversalActiveStats.rows.length}
                </div>
                <div className="text-[10px] font-mono text-zinc-600">
                  signalled ~15:50 · entered ~16:00 · {reversalExitClass} exit not yet reached
                </div>
              </div>
                ...
                <table className="min-w-[900px] w-full text-xs font-mono">
                  <thead className="sticky top-0 z-10 border-b border-white/[0.08] bg-[#0a0a0a]/55 text-zinc-400 backdrop-blur-xl">
                    <tr>
                      ...
                      <th className="text-right p-2.5">Mins To Exit</th>
                      <th className="text-right p-2.5 border-l border-white/10">Entry Stack%</th>
                      <th className="text-right p-2.5">Last Stack%</th>
                      <th className="text-right p-2.5">Signal Dev</th>
                      <th className="text-right p-2.5">P&amp;L ($)</th>
                      <th className="text-right p-2.5 border-l border-white/10">Gamma × N</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reversalActiveStats.rows.map((r, i) => (
                      <tr
                    ...
                    {!reversalActiveStats.rows.length && (
                      <tr>
        ...
        {primaryPanel === "scanner" && (tab === "analytics" || (isStreamOnlyShell && tab === "episodes")) && (
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between mb-2">
                <div className="text-[10px] uppercase tracking-widest font-mono text-zinc-500">
                  REALIZED TRADES | rows {reversalSnapshotStats.rows.length} | size ${sizeValue}/trade
                </div>
              </div>
                ...
                <table className="min-w-[1100px] w-full text-xs font-mono">
                  <thead className="sticky top-0 z-10 border-b border-white/[0.08] bg-[#0a0a0a]/55 text-zinc-400 backdrop-blur-xl">
                    <tr>
                      ...
                      <th className="text-right p-2.5 border-l border-white/10">Entry Stack%</th>
                      <th className="text-right p-2.5">Exit Stack%</th>
                      <th className="text-right p-2.5">Move (pts)</th>
                      <th className="text-right p-2.5">P&amp;L ($)</th>
                      <th className="text-right p-2.5 border-l border-white/10">Signal Dev</th>
                      <th className="text-right p-2.5 border-l border-white/10">Gamma × N</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reversalSnapshotStats.rows.map((r, i) => (
                      <tr
                    ...
                    {!reversalSnapshotStats.rows.length && (
                      <tr>
                        <td colSpan={10} className="p-8 text-center text-zinc-500">
                          {reversalSnapshotLoading ? "Building tape replay…" : "No realized trades for this date range/gates."}
                        </td>
                      </tr>
                    )}
                  </tbody>
