...
const SONAR_NAV = getLiveStrategy("daytwo")!.nav;
...
const IGNORE_LS_KEY = "bridge.daytwo.ignoreTickers.v2";
const APPLY_LS_KEY = "bridge.daytwo.applyOnlyTickers.v1";
const PIN_LS_KEY = "bridge.daytwo.pinTickers.v1";
const ACTIVE_PANEL_LS_KEY = "bridge.daytwo.activePanel.v1";
const UI_STATE_LS_KEY = "bridge.daytwo.uiState.v1";
...
const SONAR_ACTIVE_PRESET_ID_LS_KEY = "daytwo.sonar.shared-preset.active-id";
...
export default function OpenDoorSonar() {
  ...
  // The bridge already decided which tickers are listed (dayTwoSonarRows) and sent their full rows; this
  // only narrows the detail pool to them. No filter runs in the browser.
  useEffect(() => {
    if (isEditingRef.current) return;
    const listed = new Set(dayTwoSonarRows.map((r) => String(r.ticker ?? "").toUpperCase().trim()));
    const narrowed = allItems.filter((s) => listed.has(String(s.ticker ?? "").toUpperCase().trim()));
    startTransition(() => {
      setItems(narrowed);
    });
  }, [allItems, dayTwoSonarRows, isEditing]);
  ...
  return (
      ...
      <div className="relative z-10 max-w-[1920px] mx-auto space-y-4">
        ...
        <ScannerHeader
          scannerShellTitle="DAYTWO SONAR"
