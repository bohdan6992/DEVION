...
export default function OpenFadeSonar() {
  ...
  const matchOpenDoor = useCallback((s: ArbitrageSignal): { up: boolean; down: boolean } => {
    // OpenFade's rule, in place of the bins: |sigma| inside the band, and the SIGN picks the side.
    ...
    const band = { up: inBand(rawUp, true), down: inBand(rawDown, false) };
    ...
    // Deliberately NOT depending on the bin state any more: OpenFade does not consult it, and a
