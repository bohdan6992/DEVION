...
import {
  fetchOpenFadeSonarSnapshot,
  pushOpenFadeSonarLiveParams,
  toOpenFadeSonarLiveParams,
  type OpenFadeSonarRow,
} from "@/lib/sonar/openFadeSnapshotClient";
...
export default function OpenFadeSonar() {
  ...
  /** What OpenFadeSonarSnapshotService's OpenFadeLiveEngine call approved this poll — the
   * push/fetch effects live further down, since they need filter toolbar state and setError. */
  const [openFadeSonarRows, setOpenFadeSonarRows] = useState<OpenFadeSonarRow[]>([]);
  ...
  // The bridge already decided which tickers are listed (openFadeSonarRows) and sent their full rows; this
  // only narrows the detail pool to them. No filter runs in the browser.
  useEffect(() => {
    if (isEditingRef.current) return;
    const listed = new Set(openFadeSonarRows.map((r) => String(r.ticker ?? "").toUpperCase().trim()));
    const narrowed = allItems.filter((s) => listed.has(String(s.ticker ?? "").toUpperCase().trim()));
    startTransition(() => {
      setItems(narrowed);
    });
  }, [allItems, openFadeSonarRows, isEditing]);
   ...
   * uses. OpenFadeSonarSnapshotService reuses the SAME OpenFadeLiveEngine the Stream tab trades
   * on (band-only — see OpenFadeSonarLiveParamsService), so this is the exact params object
   * OpenFadeServerStrategy would build, just from the Sonar's own toolbar.
   */
  useEffect(() => {
    if (!uiHydratedRef.current) return;
    const timer = window.setTimeout(() => {
      void pushOpenFadeSonarLiveParams(toOpenFadeSonarLiveParams({
  ...
  useEffect(() => {
    let alive = true;
    const unsubscribe = subscribeSharedPoll("sonar-openfade-snapshot", fetchOpenFadeSonarSnapshot, 6_000, (value, err) => {
      if (!alive) return;
      if (err) return;
      if (value?.timedOut) {
      ...
      setOpenFadeSonarRows(value?.rows ?? []);
  ...
  // OpenFade: which tickers pass, now decided server-side (OpenFadeSonarSnapshotService, the SAME
  // OpenFadeLiveEngine the Stream tab trades on — band only) instead of matchOpenDoor's
  // client-side band check — split into two columns, SHORT (down) on the left, LONG (up) on the
  // right. This replaces the generic benchmark/beta grid below (still present, just disabled) as
  // Sonar's primary OpenFade view. matchOpenDoor is left in place above (not removed), same
  // "public export, do not prune" stance taken for applyExactSonarClientFilters elsewhere in this
  // file.
  // allItems, not items: the backend already decided which tickers pass (openFadeSonarRows), so
  ...
  const openDoorMatchedDown = useMemo(
    () => openFadeSonarRows.filter((r) => r.side === "Short"),
    [openFadeSonarRows]
  );
  const openDoorMatchedUp = useMemo(
    () => openFadeSonarRows.filter((r) => r.side === "Long"),
    [openFadeSonarRows]
  );
