...
import {
  fetchReversalSonarSnapshot,
  pushReversalSonarLiveParams,
  toReversalSonarLiveParams,
  type ReversalSonarRow,
} from "@/lib/sonar/reversalSnapshotClient";
...
import { clampNumber, optNumOrNull } from "../../lib/scanner/format";
...
const SONAR_NAV = getLiveStrategy("reversal")!.nav;
...
const IGNORE_LS_KEY = "bridge.reversal.ignoreTickers.v2";
const APPLY_LS_KEY = "bridge.reversal.applyOnlyTickers.v1";
const PIN_LS_KEY = "bridge.reversal.pinTickers.v1";
const ACTIVE_PANEL_LS_KEY = "bridge.reversal.activePanel.v1";
const UI_STATE_LS_KEY = "bridge.reversal.uiState.v1";
...
const SONAR_ACTIVE_PRESET_ID_LS_KEY = "reversal.sonar.shared-preset.active-id";
 ...
 * (ReversalSonarLiveParams.ExitClass) expects them. Same array ReversalScanner.tsx already
 * established (REVERSAL_EXIT_CLASSES there) — kept as its own local copy here, same as DayTwo keeps
...
export default function ReversalSonar() {
  const { theme } = useUi();
  ...
  // ReversalGate.Check's whole rule: |15:50 deviation| must clear BOTH a flat floor (split by side
  // — see below) AND the ticker's own published gamma for (class, sign) — see ReversalGate.cs. No
  // per-param STACK/BENCH/DEV toggles and no per-direction rate/total/move bins: Reversal has
  // neither, unlike the OpenDoor family this file was copied from — see the four-box GATE controls
  // further down, ported 1-to-1 from ReversalScanner.tsx's own pattern (AGENTS.md's spinner-input
  // standard). Naming/defaults match ReversalScanner.tsx's own reversalMinDevAbsShort/Long/Max/
  // reversalMinGammaTotal exactly, so buildReversalParams-equivalent below stays a byte-for-byte
  // mirror of the Scanner's own request builder.
  const [reversalMinDevAbsShort, setReversalMinDevAbsShort] = useState(0.5);
  // Empty means "the same threshold as the short one" (optNumOrNull(...) ?? reversalMinDevAbsShort).
  ...
  // Sample-size floor on the ticker's own published gamma (ReversalGammaEntry.GammaN). 0 (or below)
  // turns this off.
  ...
  // False outside ReversalTiming.Default's 15:45-15:55 NY signal window — see
  // ReversalLiveResult.InWindow. Distinct from `error`: this is not a fetch failure, just "there is
  ...
  // The bridge already decided which tickers are listed (reversalSonarRows) and sent their full rows; this
  // only narrows the detail pool to them. No filter runs in the browser.
  useEffect(() => {
    if (isEditingRef.current) return;
    const listed = new Set(reversalSonarRows.map((r) => String(r.ticker ?? "").toUpperCase().trim()));
    const narrowed = allItems.filter((s) => listed.has(String(s.ticker ?? "").toUpperCase().trim()));
    startTransition(() => {
      setItems(narrowed);
    });
  }, [allItems, reversalSonarRows, isEditing]);
   ...
   * OpenDoor-family Sonar panels already use. ReversalSonarSnapshotService reuses the SAME
   * ReversalLiveEngine a future ReversalServerStrategy would trade on, so this is the exact params
   * object that strategy would build, just from the Sonar's own toolbar instead of a Stream tab's
   * (which does not exist yet for Reversal).
   */
  useEffect(() => {
    if (!uiHydratedRef.current) return;
    const timer = window.setTimeout(() => {
      void pushReversalSonarLiveParams(toReversalSonarLiveParams({
  ...
  useEffect(() => {
    let alive = true;
    const unsubscribe = subscribeSharedPoll("sonar-reversal-snapshot", fetchReversalSonarSnapshot, 6_000, (value, err) => {
      if (!alive) return;
      if (err) return;
      if (value?.timedOut) {
      ...
      setReversalSonarRows(value?.rows ?? []);
      setReversalInWindow(value?.inWindow ?? true);
  ...
  // Reversal: which tickers pass, decided server-side (ReversalSonarSnapshotService, the SAME
  // ReversalLiveEngine a future ReversalServerStrategy would trade on) — split into two columns,
  // SHORT on the left, LONG on the right. This replaces the generic benchmark/beta grid below
  // (still present, just disabled) as Sonar's primary Reversal view.
  // allItems, not items: the backend already decided which tickers pass (reversalSonarRows), so
  ...
  const hasAny = benchBlocks.some((b) => b.buckets.some((g) => g.rows.length > 0))
    || reversalMatchedLong.length > 0 || reversalMatchedShort.length > 0;
      ...
      <div className="relative z-10 max-w-[1920px] mx-auto space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-white/[0.06] bg-[#0a0a0a]/50 p-3 shadow-xl backdrop-blur-md transition-all duration-300 hover:border-white/[0.12] hover:bg-[#0a0a0a]/70">
          <div className="flex h-7 items-center gap-2">
            {REVERSAL_EXIT_CLASSES.map(({ key: c, label: lbl }) => (
              <FilterButton
                key={c}
                active={reversalExitClass === c}
                label={lbl}
                onClick={() => setReversalExitClass(c)}
              />
            ))}
          </div>
        ...
        {/* Shared with the other Sonar, both Scanners and Stream — see components/shared/filters.
            ...
            nothing. ArbitrageSonar still has it. Reversal DOES use strategySlot, though (below) —
            unlike OpenDoor's DEV gate, ReversalGate.Check's floor/cap genuinely drives what this
            Sonar shows, ported 1-to-1 from ReversalScanner.tsx's own strategySlot placement. */}
        <FilterFlagsRow
          exclusions={[
        ...
        {!error && !loading && !reversalInWindow && (
          <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-center text-[10px] font-mono uppercase tracking-widest text-amber-400/80">
            Outside Reversal's signal window (15:45–15:55 NY) — nothing to evaluate right now.
          </div>
        )}
        ...
        {!error && !loading && (
          <div className="grid grid-cols-2 gap-4">
            {[
              { dir: "down" as const, title: "SHORT", accent: "text-rose-400", border: "border-rose-500/20", rows: reversalMatchedShort },
              { dir: "up" as const, title: "LONG", accent: "text-[#6ee7b7]", border: "border-emerald-500/20", rows: reversalMatchedLong },
            ].map((col) => (
              <div key={col.dir} className={clsx("rounded-2xl border bg-[#0a0a0a]/40 p-3", col.border)}>
                ...
                {col.rows.length === 0 ? (
                  <div className="py-6 text-center text-[11px] font-mono uppercase tracking-widest text-zinc-600">
                    {reversalInWindow ? "No tickers pass current Reversal gate" : "Outside signal window"}
                  </div>
                ) : (
