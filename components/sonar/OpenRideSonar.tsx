...
import {
  fetchOpenRideSonarSnapshot,
  pushOpenRideSonarLiveParams,
  toOpenRideSonarLiveParams,
  type OpenRideSonarRow,
} from "@/lib/sonar/openRideSnapshotClient";
...
const SONAR_NAV = getLiveStrategy("openride")!.nav;
...
const IGNORE_LS_KEY = "bridge.openride.ignoreTickers.v2";
const APPLY_LS_KEY = "bridge.openride.applyOnlyTickers.v1";
const PIN_LS_KEY = "bridge.openride.pinTickers.v1";
const ACTIVE_PANEL_LS_KEY = "bridge.openride.activePanel.v1";
const UI_STATE_LS_KEY = "bridge.openride.uiState.v1";
...
const SONAR_ACTIVE_PRESET_ID_LS_KEY = "openride.sonar.shared-preset.active-id";
...
/* =========================
   COMPONENT
========================= */
export default function OpenRideSonar() {
  const { theme } = useUi();
  ...
  // OpenRide selects on the stack's sigma deviation, not on a rating bin. The Sonar has to apply
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const { getOpenRideList } = await import("@/lib/trapClient");
  ...
  const matchOpenDoor = useCallback((s: ArbitrageSignal): { up: boolean; down: boolean } => {
    // OpenRide's rule, in place of the bins: |sigma| inside the band, and the SIGN picks the side
    // — positive buys, negative sells.
    // Each side reads its own live field — the Ask-priced sigma for a buy, the Bid-priced one for
    // a sell — matching TapeOpenDoorEngine.FadePass on the bridge.
    // Percent reads the raw zap fields, sigma the normalised ones — the same split the tape makes
    // between ZapPctL/S and SigmaZapL/S.
    const values = readOpenDoorGateValues(s);
    const rawUp = fadeMetric === "pct" ? Number((s as any)?.zapL) : values.devUp;
    const rawDown = fadeMetric === "pct" ? Number((s as any)?.zapS) : values.devDown;
    ...
    // Sign reversed against OpenRide's twin: this strategy rides the move, so the positive
    // reading is the buy. See TapeOpenDoorEngine.FadePass(ride: true).
    const band = { up: inBand(rawUp, false), down: inBand(rawDown, true) };
    if (openDoorIgnoreRatings) return band;

    // Gate on: the ticker's own bins must also clear, exactly as TapeOpenDoorEngine does before it
    // records an entry. Showing band-only rows here would list tickers the Stream will not send.
    const gate = matchOpenDoorGate(
    ...
    return { up: band.up && gate.up, down: band.down && gate.down };

    // Deliberately NOT depending on the bin state any more: OpenRide does not consult it, and a
    // stale dependency here would re-run the filter on edits that change nothing.
  }, [
    fadeMetric, fadeMinAbs, fadeMaxAbs,
    openDoorIgnoreRatings, openDoorBestByTicker, openDoorExitClass, openDoorAdvancedMode,
  ...
  /** What OpenRideSonarSnapshotService's OpenRideLiveEngine call approved this poll — the
   * push/fetch effects live further down, since they need filter toolbar state and setError. */
  const [openRideSonarRows, setOpenRideSonarRows] = useState<OpenRideSonarRow[]>([]);
  ...
  /**
   * The toolbar, to the bridge — same debounce/hydration-guard pattern the rest of this cutover
   * uses. OpenRideSonarSnapshotService reuses the SAME OpenRideLiveEngine the Stream tab trades
   * on.
   *
   * OPERATOR DECISION, 2026-09-13: this panel's OWN matchOpenDoor above combines the band test
   * AND matchOpenDoorGate (unless ignoreRatings) — stricter than what the Stream actually trades.
   * The backend selects band-only, matching OpenRideLiveEngine.BandPass, so the rows below can
   * show MORE tickers than matchOpenDoor's local up/down columns would (that function and
   * matchOpenDoorGate are left in place, not removed — see OpenRideSonarLiveParamsService.cs).
   */
  useEffect(() => {
    if (!uiHydratedRef.current) return;
    const timer = window.setTimeout(() => {
      void pushOpenRideSonarLiveParams(toOpenRideSonarLiveParams({
        exitClass: openDoorExitClass,
        useStack: openDoorUseStack,
        useBench: openDoorUseBench,
        useDevSig: openDoorUseDevSig,
    ...
    openDoorUpMinRate, openDoorUpMinTotal, openDoorUpMinMove,
    openDoorDownMinRate, openDoorDownMinTotal, openDoorDownMinMove,
    fadeMetric, fadeMinAbs, fadeMaxAbs, openDoorIgnoreRatings,
    listMode, ignoreSet, applySet, pinMap, activeMode,
    includeUSA, includeChina, selCountries, countryEnabled,
    selExchanges, exchangeEnabled, selSectors, sectorEnabled,
    bounds, excludeDividend, excludeNews, excludePTP, excludeSSR,
  ...
  useEffect(() => {
    let alive = true;
    const unsubscribe = subscribeSharedPoll("sonar-openride-snapshot", fetchOpenRideSonarSnapshot, 6_000, (value, err) => {
      if (!alive) return;
      if (err) return;
      if (value?.timedOut) {
  ...
  // instead of matchOpenDoor's client-side band+gate check — split into two columns, SHORT (down)
  // on the left, LONG (up) on the right. This replaces the generic benchmark/beta grid below
  // (still present, just disabled) as Sonar's primary OpenRide view. matchOpenDoor/
  // matchOpenDoorGate are left in place above (not removed), same "public export, do not prune"
  // stance taken for applyExactSonarClientFilters elsewhere in this file.
  // allItems, not items: the backend already decided which tickers pass (openRideSonarRows), so
  ...
  const openDoorMatchedDown = useMemo(
    () => openRideSonarRows.filter((r) => r.side === "Short"),
    [openRideSonarRows]
  );
  const openDoorMatchedUp = useMemo(
    () => openRideSonarRows.filter((r) => r.side === "Long"),
    [openRideSonarRows]
  );
  ...
  return (
      ...
      <div className="relative z-10 max-w-[1920px] mx-auto space-y-4">
        ...
        <ScannerHeader
          scannerShellTitle="OPENRIDE SONAR"
