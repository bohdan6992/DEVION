...
const UI_STATE_LS_KEY = "bridge.opendoor.uiState.v1";
...
export default function OpenDoorSonar() {
  ...
  // The bridge already decided which tickers are listed (openDoorSonarRows) and sent their full rows; this
  // only narrows the detail pool to them. No filter runs in the browser.
  useEffect(() => {
    if (isEditingRef.current) return;
    const listed = new Set(openDoorSonarRows.map((r) => String(r.ticker ?? "").toUpperCase().trim()));
    const narrowed = allItems.filter((s) => listed.has(String(s.ticker ?? "").toUpperCase().trim()));
    startTransition(() => {
      setItems(narrowed);
    });
  }, [allItems, openDoorSonarRows, isEditing]);
  ...
  // allItems, not items: the backend already decided which tickers pass (openDoorSonarRows), so
