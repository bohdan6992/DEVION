      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-8">
        ...
        <div className="rounded-2xl p-6 bg-neutral-800/60 shadow-xl ring-1 ring-white/5">
          <h2 className="text-xl font-semibold mb-4">
            {editing ? "Редагувати стратегію" : "Нова стратегія"}
          </h2>
          ...
          <form onSubmit={save} className="grid md:grid-cols-2 gap-3">
            ...
            <input
              className="px-3 py-2 rounded bg-neutral-900 border border-neutral-700"
              placeholder="Icon URL (опційно)"
              value={form.icon || ""}
              onChange={(e) => setForm({ ...form, icon: e.target.value })}
            />

            <textarea
              className="md:col-span-2 px-3 py-2 rounded bg-neutral-900 border border-neutral-700"
            ...
            <div className="md:col-span-2 flex items-center gap-3">
              {form.icon ? (
                <a className="underline" href={form.icon} target="_blank" rel="noreferrer">
                  переглянути іконку
                </a>
              ) : (
                <span className="text-neutral-400">Іконка не вказана</span>
              )}
            </div>
              ...
              <tbody>
                {strategies.map((s) => (
                  <tr key={s.id} className="border-b border-neutral-800 hover:bg-neutral-800/40">
                    ...
                    <td>
                      {s.icon ? (
                        <a className="underline" href={s.icon} target="_blank" rel="noreferrer">
                          іконка
                        </a>
                      ) : (
                        "—"
                    ...
                    <td className="flex gap-2 py-2">
                      ...
                      <button className="btn" onClick={() => { setEditing(s); setForm(s); }}>
                        Редагувати
                      </button>
